...
class APIRouter:
    ...
    def route(self, method, path, payload):
        ...
        if pure=='/api/version': return 200, self.connector.get_version_info()
        if pure=='/api/contracts': return 200, self.connector.get_api_contracts()
        if pure=='/api/contracts/capabilities': return 200, self.connector.get_api_contracts().get('capabilities', {})
